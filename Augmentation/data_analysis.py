...
def colorize_mask(mask):
    ...
    color_map = {val: np.random.randint(0, 255, size=3) for val in unique_values if val != 0}  # Exclude 0 (background)

    # Create an RGB mask
    colorized_mask = np.zeros((*mask.shape, 3), dtype=np.uint16)
    print(colorized_mask.shape)

    for val, color in color_map.items():
        colorized_mask[mask == val] = color  # Assign the unique color
...
def display_tif_images(brightfield_folder, masks_folder, display_percentage=10):
    ...
    for i, filename in enumerate(common_files[:num_to_display]):
        ...
        if len(mask_image.shape) == 2:
            colorized_mask = colorize_mask(mask_image)
            cv2.imwrite(mask_path, cv2.cvtColor(colorized_mask, cv2.COLOR_RGB2BGR))
        else:
            colorized_mask = mask_image
        ...
        plt.title(f"Colorized Mask: {filename}", fontsize=8)
        plt.axis('off')
...
def display_tif_images_2(brightfield_folder, masks_folder, display_percentage=10):
    # Get all .tif files
    brightfield_files = [f for f in os.listdir(brightfield_folder) if f.endswith('.tif')]
    mask_files = [f for f in os.listdir(masks_folder) if f.endswith('.tif')]
